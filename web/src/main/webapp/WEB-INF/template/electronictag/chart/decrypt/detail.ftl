...
<body>
					...
					<div class="chartbox">
						<div class="chartitem">
							<div class="chart-header">
								<span class="title"><i class="icon icon-chart"></i>客户端解密数量总数：</span>
								<span class="total">0</span>个
							</div>
							<div class="chart-body">
								<div id="decrypt_file" width="100%" height="100%"></div>
							</div>
						</div>
					</div>
					<div class="chartbox">
						<div class="chartitem">
							<div class="chart-header">
								<span class="title"><i class="icon icon-chart"></i>审批流程解密数量总数：</span>
								<span class="total">0</span>个
							</div>
							<div class="chart-body">
								<div id="flow_decrypt_file" width="100%" height="100%"></div>
							</div>
						</div>
					</div>
	...
	function zTreeOnClick(event, treeId, treeNode){
		...
		pieInit({
			id: "decrypt_file",
			url: "${base}/admin/chart_detail/decrypt/pie/department.jhtml?id=" + departmentId
		});
		pieInit({
			id: "flow_decrypt_file",
			url: "${base}/admin/chart_detail/decrypt/flow/pie/department.jhtml?id=" + departmentId
		});
		refreshPage();
		refreshPageFlow();
	}
	...
	$(document).ready(function(){
		...
		pieItem({id:"decrypt_file",url:"${base}/admin/chart_detail/decrypt/pie/department.jhtml?id=" + departmentId});
		pieItem({id:"flow_decrypt_file",url:"${base}/admin/chart_detail/decrypt/flow/pie/department.jhtml?id=" + departmentId});
		resizeItemById("decrypt_file");
		resizeItemById("flow_decrypt_file");
		$("input[name='id']").val(departmentId);
		refreshPage();
		refreshPageFlow();
	});
